...
- **Never delete `.astro/` (or run builds that do) while the dev server is running**: it orphans the background-mode dev server (still listening but untracked and serving stale module graphs — all client animations appear dead). Stop the dev server first or leave `.astro/` alone; deleting `dist/` before builds is fine.
...
- Folders are split by **concern**, inside `src/`:
  ...
  | `src/scripts/` | Scripts vanilla de cliente que cargan los componentes (`header.ts` estado del header, `lenis.ts` smooth scroll global con Lenis) | `.ts` |
...
- Animations (GSAP) are per-component, no global bootstrap script. `Reveal.tsx` (scroll reveal with ScrollTrigger, `as`/`delay`/`trigger` props; `trigger="mount"` for entrance-on-load) and `Marquee.tsx` (loop marquee, `direction`/`speed` props) are the shared animated primitives. `Header.astro` loads a small vanilla script (`src/scripts/header.ts`) for the `scrolled` state (no React needed for that). **One global exception**: `src/scripts/lenis.ts` (imported from `Layout.astro`) boots Lenis smooth scroll on every page — it drives `ScrollTrigger.update` from Lenis' scroll events and its raf runs on the GSAP ticker (`lagSmoothing(0)`); it also intercepts same-page anchor links (e.g. `/#contacto`) with `lenis.scrollTo(section, { offset: -96 })` — don't rely on CSS `scroll-behavior` (removed from `base.css` on purpose).
...
- **Reglas de componente**: una responsabilidad por archivo; `ui/` sin lógica de negocio; secciones no importan secciones; `client:load` solo sobre el primitivo que anima (no sobre wrappers estáticos). **Comentario de cabecera obligatorio**: las primeras líneas de cada componente (`.astro` dentro del frontmatter, `.tsx`/`.ts` como primer `//`) deben explicar en español qué hace el componente.
- Fancy UI primitives: `CardStack.tsx` (deck of stacked cards with GSAP — `items`/`maxVisible`/`cardWidth`/`overlap`/`spreadDeg`/`autoAdvance`/`intervalMs`/`pauseOnHover`/`showDots` props; fan geometry + swipe-to-change with GSAP, dots below to select), `TextLoop.tsx` (text flowing along an SVG path — `shape` circle/infinity/arch/line/wave, GSAP tween on `startOffset`), `Grainient.tsx` (animated WebGL gradient, requires `ogl`, renders a fullscreen triangle with a fragment shader, pauses when off-screen), `StickerTrail.tsx` (cursor-following sticker trail — spawns cloned `<img>`s from `/stickers/` in `public/` with the Web Animations API), `HorizontalGallery.tsx` (GSAP demo "horizontal scrolling gallery" replica — `items: GalleryPanel[]` prop; pins the wrapper with ScrollTrigger (`start: "center center"`, `end: "+=strip.scrollWidth"`, `scrub`) while the strip translates in X by `-(scrollWidth - innerWidth)`; section must be full-bleed, not inside `Container`). The bouncy footer wave is **vanilla** (not React): SVG + ScrollTrigger/MorphSVGPlugin live in `Footer.astro` inner svg (`id="bouncy-path"`) and `src/scripts/footer-wave.ts` (trigger `.footer`, `start: "top bottom"`, elastic by `getVelocity()/10000`).
